...
import torch
from control.pitching_ball_mound.pitching01_ppo_mp import PPO
...
def main():
    ...
    if not MOTION_ONLY:
        ...
        # ppo.LoadModel('pitching01_model_202309082236/' + 'max.pt')
        # ppo.LoadModel('pitching01_model_202309131311/' + 'max.pt')
        # ppo.LoadModel('pitching01_model_202308261215/' + '00531.pt')
    ...
    if not MOTION_ONLY:
        ...
        ##
        viewer.doc.addRenderer("bicep", yr.TrajRenderer(bicep_size, bicep_T, colors=render_color))
        viewer.doc.addRenderer('forearm', yr.TrajRenderer(forearm_size, forearm_T, colors=render_color))
        viewer.doc.addRenderer('hand', yr.TrajRenderer(hand_size, hand_T, colors=render_color))
    ...
    def simulateCallback(frame):
        ...
        if frame%10 == 0 or 100 > frame > 75:
            ...
            # on mound
            bicep_size.append(currentSkelState[bicep_idx][3])
            bicep_T.append(currentSkelState[bicep_idx][2])

            forearm_size.append(currentSkelState[forearm_idx][3])
            forearm_T.append(currentSkelState[forearm_idx][2])
            
            hand_size.append(currentSkelState[hand_idx][3])
            hand_T.append(currentSkelState[hand_idx][2])
