...
import numpy as np
from math import exp, pi, log, acos, sqrt, sin, cos
...
class SkateDartEnv(gym.Env):
    def __init__(self, ):
        ...
        # self.world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_mound_test.skel')
        self.world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_test.skel')

        self.world.control_skel = self.world.skeletons[1]
        self.skel = self.world.skeletons[1]
        self.Kp, self.Kd = 600., 49.

        # self.ref_world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_mound_test.skel')
        self.ref_world = pydart.World(1./150., cur_path+'/../../data/skel/human_mass_limited_dof_v2_ball_test.skel')
        self.ref_skel = self.ref_world.skeletons[1]
        self.ref_motion = DartSkelMotion()
        self.ref_motion.load(cur_path + '/pitching01.skmo')
        self.ref_motion.reset_root_trajectory_pitching01(self.ref_skel)
        # self.ref_motion.reset_root_trajectory_pitching01_vibe_on_mound(self.ref_skel)
        ...
        # self.ball.body('ball').set_collidable(True)
        ...
        self.rf_nontact_start_frame1 = 47
        self.rf_nontact_end_frame1 = 85

        self.lf_nontact_start_frame1 = 87
        self.lf_nontact_end_frame1 = 111
        ...
        for json_file in sorted(glob.glob(json_path + "*.json")):
            ...
            if json_file == json_path+"pitching01_000000000120_keypoints.json":
                for i in range(10):
                    self.up_angle_list.append(up_angle)
    ...
    def reward(self):
        ...
        skel_pos_diff = self.skel.position_differences(self.skel.q, self.ref_skel.q)
        skel_pos_diff[16:28] = 0.0*skel_pos_diff[16:28]

        # r_p = exp_reward_term(self.w_p, self.exp_p, self.skel.position_differences(self.skel.q, self.ref_skel.q)[6:] / sqrt(
        #                           len(self.skel.q[6:])))

        r_p = exp_reward_term(self.w_p, self.exp_p, skel_pos_diff[6:] / sqrt(len(self.skel.q[6:])))
        
        skel_vel_diff = self.skel.velocity_differences(self.skel.dq, self.ref_skel.dq)
        skel_vel_diff[16:28] = 0.0 * skel_vel_diff[16:28]
        # r_v = exp_reward_term(self.w_v, self.exp_v, self.skel.velocity_differences(self.skel.dq, self.ref_skel.dq)[6:] / sqrt(
        #                           len(self.skel.dq[6:])))

        r_v = exp_reward_term(self.w_v, self.exp_v, skel_vel_diff[6:] / sqrt(len(self.skel.dq[6:])))
        
        # print(self.ref_motion.get_q(self.current_frame))
        ...
        if self.ball_released_frame < self.current_frame:
            ...
            # print("self.ball.com(): ", self.ball.com())
            for future_frame in range (self.ball_released_frame+1, self.motion_len):
                dt = (future_frame - self.current_frame) / self.ref_motion.fps
                expected_ball_pos = self.ball.com() + dt*self.ball.com_velocity() + 0.5*dt*dt*self.world.gravity()
                if expected_ball_pos[2] < self.target_zone_pos[2]:
                    expected_diff = self.target_zone_pos - expected_ball_pos
                    # print("expected_diff: ", future_frame, expected_diff)
                    dis = np.linalg.norm(expected_diff)
                    # print("dis: ", dis)
                    dis_list.append(dis)
    ...
    def step(self, _action):
        ...
        #     self.world.step()
        ##
        # left_hand_pos = self.skel.q
        # print(self.skel.q[16])
        del self.step_torques[:]
        for i in range(self.step_per_frame):
       ...
       # ground: 0
        ...
        # print(self.contact_label[self.current_frame][0])

        if self.lf_nontact_start_frame1 <= self.current_frame <= self.lf_nontact_end_frame1:
            self.p_fc_hat[0] = 0
        elif self.rf_nontact_start_frame1 <= self.current_frame <= self.rf_nontact_end_frame1:
            self.p_fc_hat[1] = 0
        else:
            self.p_fc_hat[0] = 1
            self.p_fc_hat[1] = 1

        # if self.current_frame > 85:
        #     self.p_fc_hat[1] = 2

        # if self.current_frame > 112:
        #     self.p_fc_hat[0] = 2
        #     self.p_fc_hat[1] = 2
    ...
    def reset(self):
        ...
        if self.weld_constraint is None:
            self.weld_constraint = pydart.constraints.WeldJointConstraint(self.skel.body('h_forearm_left'), self.ball.body(0))
